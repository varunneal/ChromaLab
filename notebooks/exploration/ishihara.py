...
class IshiharaPlate:
    ...
    def export_plate(self, save_name: str, ext = None):
        """
        This method saves a .tetra file as well as 4 grayscale images (one
        for each of the R, G1, G2, B channels).

        :param save_name: Name of directory to save plate to.
        :param ext: File extension to use, such as 'png' or 'tif'.
        """
        save_dir = f'{self.directory}/{save_name}'
        os.makedirs(save_dir, exist_ok=True)

        # Save .tetra file.
        r, g1, g2, b = [np.asarray(ch) for ch in self.channels]
        tetra_data = np.dstack([r, g1, g2, b])
        np.save(f'{save_dir}/{save_name}.tetra', tetra_data)

        # Save 4 grayscale images.
        if ext:
            ch_names = ['R', 'G1', 'G2', 'B']
            for i, ch in enumerate(self.channels):
                ch.save(f'{save_dir}/{save_name}_{ch_names[i]}.{ext}')
    ...
    def __reset_images(self):
        ...
        """
        self.channels = [Image.new(mode='L', size=(self.image_size, self.image_size)) for _ in range(4)]
